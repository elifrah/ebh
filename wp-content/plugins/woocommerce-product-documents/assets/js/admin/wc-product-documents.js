jQuery( document ).ready( function( $ ) {
	...
	// Make the radio buttons behave somewhat like checkboxes by unchecking when clicking a checked radio
	.on( 'click', 'input.product-documents-default-section', function() {

		var previousValue = 'true' === $( this ).attr( 'previousValue' );
		$( '#wc-product-documents-data input.product-documents-default-section' ).attr( 'previousValue', false );
	...
	// setup the default radio button "previous value" so we can uncheck it like a checkbox
	$( '#wc-product-documents-data input.product-documents-default-section' ).each( function() {
		if ( $( this ).is( ':checked' ) ) {
	...
	function updateDocumentSectionRowIndexes() {
		$( '.wc-product-documents-sections .wc-product-documents-section' ).each(
			function( index, el ) {
				$( '.product-documents-section-position', el ).val( parseInt( $( el ).index( '.wc-product-documents-sections .wc-product-documents-section' ) ) );
			}
		);
	};
	...
	function updateDocumentRowIndexes( $section ) {
		$( '.wc-product-documents .wc-product-document', $section ).each(
			function( index, el ) {
				$( '.wc-product-document-position', el ).val( parseInt( $( el ).index( '.wc-product-documents .wc-product-document' ) ) );
			}
		);
	};
	...
	function setAttachFileHandler() {
		...
		$( '#wc-product-documents-data .wc-product-documents-set-file' ).live( 'click', function( event ) {
